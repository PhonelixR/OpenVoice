# openvoice_app_fixed_lang.py - V2 soporta múltiples idiomas
import os
import torch
import argparse
import gradio as gr
        ...
        with gr.Column(scale=1):
            ...
            def update_styles(version):
                if version == "V1":
                    ...
                    # Crear opciones con descripción
                    v2_choices_with_desc = []
                    for s in v2_styles:
                        desc = v2_style_to_language.get(s, s)
                        v2_choices_with_desc.append(f"{s} - {desc}")
